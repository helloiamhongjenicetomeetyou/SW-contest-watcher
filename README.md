...
| 감시 대상 | 찾는 것 | 여기에만 올라오는 것 | 수집 방식 |
| --- | --- | --- | --- |
| [대표 홈페이지 일반공지](https://www.ulsan.ac.kr/kor/CMS/Board/Board.do?mCode=MN113) | 장학 | 국가장학금·국가근로장학, 손태희·세종이도 같은 교외 재단장학 | 게시판 **제목 검색**이 GET으로 열려서 키워드로 바로 질의 |
| [ICT융합학부 공지사항](https://ict.ulsan.ac.kr/ict/5778) | 장학 | 학부 우수장학, 봉사장학(트랙), 역량개발·빛냄장학 | 검색이 없어서 최근 2페이지를 훑어 제목 매칭 |
| [SW중심대학사업단 공지](https://sw.ulsan.ac.kr/site/swulsan/notices) | 대회 | 해커톤, 경진대회, 공모전, 챌린지 | 공개 **JSON API** 한 번 |

셋은 서로 겹치지 않습니다. `ICT융합학부 우수장학 서류 제출 안내`는 대표 홈페이지에 안 올라오고,
`국가근로장학금 신청기간 안내`는 학부 공지에 안 올라오고, `2026 부울경 AI융합 해커톤`은 SW사업단에만
올라오는 식입니다. 메일 제목 앞에 `[울산대 장학]` / `[ICT 장학]` / `[SW 대회]`가 붙어서
어느 게시판에서 온 건지 바로 구분됩니다.

**SW사업단 게시판만 다른 방식으로 가져옵니다.** 이 사이트는 학교 전산망이 아니라 CloudFront 뒤에 있고
게시판 데이터가 외부 CMS의 공개 JSON API로 나옵니다. HTML을 긁지 않으니 화면이 바뀌어도 잘 안 깨지고,
학교 서버처럼 요청을 거절당하지도 않습니다.

대회 공고 중 **`수상자`·`명단`·`최종 결과`가 제목에 든 글은 뺍니다.** 이미 끝난 소식이라
`CONTEST_EXCLUDE`에 넣어뒀습니다. 남의 수상 소식까지 받고 싶으면 그 목록을 비우면 됩니다.
...
2. 세 게시판에서 후보 글을 모아 제목이 **그 게시판의 키워드**에 걸리는 글을 고릅니다.
...
이때는 목록만 읽고 상세 페이지는 건너뛰어서 요청이 네 번으로 끝납니다.
게시판별로 따로 판단하므로, 나중에 학부 게시판을 하나 더 붙여도 그 게시판의 과거 글이 쏟아지지 않습니다.

학교 서버는 짧은 시간에 요청이 몰리면 연결 자체를 거절합니다(`Connection refused`).
그래서 연결을 재사용하고, 요청 사이를 3초 띄우고, 실패하면 3초·6초·12초 간격으로 세 번까지 다시 시도합니다.

그래도 본문을 못 읽으면 **제목·작성일·링크만 넣어서 메일을 보냅니다.**
본문이 빠지더라도 "새 공고가 떴다"는 알림 자체는 놓치지 않는 쪽을 택했습니다.
한 번 막히면 그 회차에는 본문 수집을 더 시도하지 않습니다.

한쪽 게시판이 죽어도(사이트 개편·점검 등) 나머지 게시판은 정상적으로 확인하고,
...
- **`ConnectTimeout` / `Connection refused`가 날 수 있습니다.** 러너는 미국에 있어서 학교 서버가 연결을 늦게 받거나 아예 거절할 때가 있습니다. 본문만 못 읽은 거면 제목과 링크로 메일이 나가고, 목록조차 못 읽으면 그 회차를 건너뛰고 2시간 뒤에 다시 시도합니다. 실패한 글은 state에 기록되지 않으므로 다음 실행 때 그대로 잡힙니다.
...
키워드는 게시판별로 따로 있습니다. `watcher.py` 위쪽에서 바꿉니다.

```python
SCHOLARSHIP_KEYWORDS = ["장학"]                        # 대표 홈페이지 + 학부 공지
CONTEST_KEYWORDS = ["대회", "공모전", "해커톤", "경진", "챌린지", "challenge"]  # SW사업단
CONTEST_EXCLUDE = ["수상자", "명단", "최종 결과", "결과 안내"]
```

기본값은 `장학` 하나라 장학이 들어간 공고가 전부 옵니다. 좁히려면 이름을 적습니다.

```python
SCHOLARSHIP_KEYWORDS = ["ICT융합학부 우수장학", "국가근로장학", "봉사장학"]
```

띄어쓰기와 대소문자는 무시하고 비교하므로 `국가 근로 장학`으로 적어도 같은 글이 걸립니다.
게시판 하나만 다르게 걸러야 하면 `BOARDS`에서 그 게시판에 `keywords=`, `exclude=`를 직접 줍니다.
