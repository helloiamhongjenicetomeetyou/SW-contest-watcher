...
## 내 학년이 나갈 수 있는 것만 옵니다

`MY_GRADE`(기본값 `1`)에 내 학년을 적어두면, **대상 학년이 적혀 있는데 내 학년이 빠져 있는 공고는
메일을 보내지 않습니다.** 3·4학년만 나갈 수 있는 취업 연계 해커톤이 1학년에게 올 이유는 없습니다.

| | 예 |
| --- | --- |
| 통과 | `학년전체`, `전 학년`, `1~4학년`, `재학생 누구나`, `참가 대상: 3학년 이하`, 학년 얘기가 아예 없는 공고 |
| 차단 | `3학년, 4학년이상`, `참가 대상: 2학년 이상`, `응모 자격: 2~4학년 재학생` |

**학년 제한을 못 찾으면 그냥 보냅니다.** 공고 대부분은 대상 학년을 안 적습니다.
못 읽었다고 빼버리면 나갈 수 있는 대회를 놓치니, 안 된다고 분명히 적힌 것만 뺍니다.

어디를 보고 판단하는지는 게시판마다 다릅니다.

- **U-STEP**은 신청 대상 학년이 `STU_TARGET_NM` 칸에 따로 있습니다(`학년전체`, `3학년, 4학년이상`).
  포스터 본문보다 정확해서 이 값만 봅니다.
- **대표 홈페이지와 SW사업단**은 본문에서 찾습니다. 제목과, `대상`·`자격`·`참가`·`응모`·`신청`·`지원`이
  들어 있는 줄만 봅니다. 본문을 통째로 훑으면 `고등학교 3학년 때` 같은 말에 걸려 멀쩡한 공고가 빠집니다.
- `2025학년도 2학기`가 `4학년`으로 읽히지 않도록, 앞에 숫자가 붙은 것과 `학년도`는 셈에서 뺍니다.

학년 때문에 거른 글은 **읽음 처리해 둡니다.** 안 그러면 두 시간마다 같은 글의 본문을 다시 읽어서
학교 서버에 괜한 요청이 들어갑니다.
...
3. `state/seen.json`에 없는 글이면 상세 페이지를 읽어 **본문·작성일·첨부파일 목록·링크**를 가져옵니다.
4. 대상 학년이 적혀 있는데 `MY_GRADE`가 빠져 있으면 거릅니다.
5. 남은 것을 메일로 보냅니다.
6. 보낸 글과 거른 글의 번호를 `state/seen.json`에 적고 리포지토리에 커밋합니다.
...
학년은 비밀이 아니라서 Secret이 아니라 `.github/workflows/watch.yml`의 `MY_GRADE`에 그대로 적혀 있습니다.
...
`--grade`는 이번 실행에만 학년을 바꿉니다. 학년 때문에 뭐가 빠졌는지 볼 때 씁니다.
...
`.github/workflows/watch.yml`의 `MY_GRADE`를 올립니다. 2학년이 되면 `"2"`.
...
`watcher.py`의 `MY_GRADE = int(os.environ.get("MY_GRADE", "1"))`이 기본값이라 환경변수를 안 주면 1학년으로 봅니다.
